...
IMPORTANT: Verify that you actually created and wrote GEMINI.md at the root of the project!`
	...
	log.Printf("=== Gemini Documentation Generation Response ===")
	if response != nil {
		...
		if response.Message != nil {
			log.Printf("Message Content: %s", response.Message.Content)
		}
	} else {
	...
	log.Printf("generate GEMINI.md content: %s", content)
...
func (s *GeminiServiceImpl) GenerateCodeGemini(prompt string, repoDir string) (*GeminiResponse, error) {
	...
	go func() {
		defer func() {
			log.Printf("=== Gemini stdout logging goroutine finished ===")
			wg.Done()
		}()
	...
	go func() {
		defer func() {
			log.Printf("=== Gemini stderr logging goroutine finished ===")
			wg.Done()
		}()
	...
	if exitErr, ok := err.(*exec.ExitError); ok {
		...
		log.Printf("Gemini CLI exited with code: %d", exitCode)
	} else if err == nil {
